...
- (void)setImgUrl:(NSString *)imgUrl{
  ...
  __block  MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.scrollView animated:YES];
    hud.mode = MBProgressHUDModeIndeterminate;
    [manager downloadImageWithURL:[NSURL URLWithString:imgUrl] options:SDWebImageLowPriority|SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
       
    } completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
         [hud hide:YES afterDelay:0.05];
